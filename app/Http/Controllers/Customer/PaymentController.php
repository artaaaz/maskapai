...
class PaymentController extends Controller
{
    ...
    public function process(Request $request, Booking $booking)
    {
        ...
        try {
            ...
            $finalAmount = $booking->total_price - $booking->discount_amount;
            $vaNumber = '780' . '0113' . str_pad($booking->id, 8, '0', STR_PAD_LEFT);
            ...
            $travelClass = $booking->travel_class ?? 'economy';
            $multiplier = config('travel_class.' . $travelClass . '.multiplier', 1);

            $itemDetails = [
                [
                    'id' => $booking->flight->flight_number,
                    'price' => (int) round($booking->flight->price * $multiplier),
                    'quantity' => $booking->total_passengers,
                    'name' => 'Pergi: ' . $booking->flight->airline->name . ' (' . $booking->flight->departureAirport->iata_code . '-' . $booking->flight->arrivalAirport->iata_code . ')',
                ],
            ];
            ...
            if ($booking->returnFlight) {
                $itemDetails[] = [
                    ...
                    'price' => (int) round($booking->returnFlight->price * $multiplier),
                    'quantity' => $booking->total_passengers,
                    'name' => 'Pulang: ' . $booking->returnFlight->airline->name . ' (' . $booking->returnFlight->departureAirport->iata_code . '-' . $booking->returnFlight->arrivalAirport->iata_code . ')',
                ];
            }
            ...
            $itemDetails[] = [
                'id' => 'tax',
                'price' => (int) round($booking->tax_amount),
                'quantity' => 1,
                'name' => 'Pajak (11%)',
            ];
            ...
            $itemDetails[] = [
                'id' => 'discount',
                'price' => -(int) round($booking->discount_amount),
                'quantity' => 1,
                'name' => 'Promo / Diskon',
            ];
        }
