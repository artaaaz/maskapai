<x-admin-layout>
    <x-slot name="header">Edit Pesawat</x-slot>
    ...
    <div class="card max-w-2xl">
        <div class="card-header"><h2>Edit Pesawat</h2></div>
        <div class="card-body">
            <form action="{{ route('admin.airplanes.update', $airplane) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="model" class="form-input @error('model') border-red-500 @enderror" value="{{ old('model', $airplane->model) }}" required>
                    @error('model') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Nomor Registrasi</label>
                    <input type="text" name="registration_number" class="form-input @error('registration_number') border-red-500 @enderror" value="{{ old('registration_number', $airplane->registration_number) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="capacity" class="form-input @error('capacity') border-red-500 @enderror" value="{{ old('capacity', $airplane->capacity) }}" min="1" required>
