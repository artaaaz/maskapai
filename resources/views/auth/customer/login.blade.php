        ...
        <div class="lg:w-1/2 flex items-center justify-center p-6 lg:p-12">
            <div class="w-full max-w-md">
                <div class="bg-white rounded-3xl shadow-xl p-8">
                    ...
                    <form method="POST" action="{{ route('customer.login.store') }}" id="loginForm" class="space-y-4">
                        @csrf
                        <div>
                    ...
                    <p class="mt-6 text-center text-sm text-slate-600">
                        Belum punya akun?
                        <a href="{{ route('customer.register') }}" class="text-blue-600 font-bold hover:underline">Daftar</a>
                    </p>
                </div>

                <p class="text-center text-xs text-slate-400 mt-6">
                    Staff? <a href="{{ route('staff.login') }}" class="underline">Login staff</a> ·
                    Manager? <a href="{{ route('manager.login') }}" class="underline">Login manager</a> ·
                    Admin? <a href="{{ route('admin.login') }}" class="underline">Login admin</a>
                </p>
            </div>
        </div>
    </div>
