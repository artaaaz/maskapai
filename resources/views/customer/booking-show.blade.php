...
<div class="bg-slate-100 min-h-screen py-8">
    <div class="max-w-4xl mx-auto px-4">
        ...
        {{--
            Status Card
            PENTING: booking.status memiliki 8 kemungkinan nilai valid:
            pending, paid, confirmed, checked_in, boarded, in_progress, completed, cancelled.
            Hanya 'pending' (menunggu) dan 'cancelled' (gagal/dibatalkan) yang BUKAN status sukses.
            Status paid/confirmed/checked_in/boarded/in_progress/completed SEMUA berarti
            pembayaran berhasil dan booking valid — jangan pernah menampilkan pesan
            "Pembayaran Gagal / Dibatalkan" untuk status-status ini.
        --}}
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl
                        @if($isSuccess) bg-green-100
                        @elseif($isPending) bg-amber-100
                    ...
                    <div>
                        <p class="text-xl font-bold text-slate-800">
                            @if($isSuccess) Pembayaran Berhasil! Tiket Aktif
                            @elseif($isPending) Menunggu Pembayaran
                            @else Pembayaran Gagal / Dibatalkan @endif
                        </p>
                        <p class="text-sm text-slate-500">
                            @if($isSuccess)
                                {{ $booking->status_badge['label'] }} &mdash; E-ticket tersedia, silakan lihat atau cetak tiket Anda.
                            @elseif($isPending)
                ...
                <span class="px-4 py-2 rounded-xl text-sm font-bold 
                    @if($isSuccess) bg-green-100 text-green-700
                    @elseif($isPending) bg-amber-100 text-amber-700
                    @else bg-red-100 text-red-700 @endif">
                    {{ strtoupper($booking->status_badge['label']) }}
                </span>
            </div>
        </div>
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <span class="font-bold text-slate-700">{{ $passenger->seat_number ?? '-' }}</span>
                                </td>
                                <td class="px-4 py-3">
