    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-slate-200 p-6">
                ...
                @php
                    $upcomingBookings = \App\Models\Booking::with(['flight.airline', 'flight.departureAirport', 'flight.arrivalAirport', 'payment'])
                    ...
                    <div class="space-y-4">
                        @foreach($upcomingBookings as $booking)
                            @php
                                ...
                                $daysLeft = now()->diffInDays($departure, false);
                                $hoursLeft = now()->diffInHours($departure, false) % 24;
                                $minutesLeft = now()->diffInMinutes($departure, false) % 60;
                            @endphp
                            <div class="border border-slate-100 rounded-xl p-4 hover:shadow-md transition-all">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="text-xs font-semibold text-white px-2.5 py-1 rounded-full
                                        @if($daysLeft <= 1) bg-red-500
                                        @elseif($daysLeft <= 3) bg-amber-500
                                        @else bg-green-500
                                        @endif">
                                        @if($daysLeft > 0)
                                            {{ $daysLeft }}h {{ $hoursLeft }}m lagi
                                        @elseif($hoursLeft > 0)
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <a href="{{ route('customer.search') }}" class="text-blue-600 text-sm hover:underline mt-1 inline-block">Cari penerbangan sekarang</a>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl shadow-md p-6 text-white">
                ...
                <div class="space-y-3">
                    @php
                        ...
                        $confirmedTrips = \App\Models\Booking::where('user_id', Auth::id())->where('status', 'confirmed')->count();
