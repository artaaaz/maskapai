            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-800 p-6 text-white">
                <div class="flex justify-between items-start">
                    ...
                    <div class="text-right">
                        <p class="text-2xl font-black">{{ $booking->flight->airline->code ?? $booking->booking_code }}-{{ $booking->booking_code }}</p>
            ...
            <div class="p-6">
                ...
                <div class="bg-slate-50 rounded-xl p-4 mb-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">{{ $booking->flight->flight_number }} • {{ $booking->flight->airplane->model ?? '' }}</p>
                        </div>
                        <div class="ml-auto text-right">
                            <p class="text-sm font-semibold text-slate-700">{{ ucfirst(str_replace('_', ' ', $booking->travel_class ?? 'Economy')) }}</p>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-blue-50 rounded-xl p-4 text-center">
                        ...
                        <p class="text-2xl font-black text-blue-700 mt-1">{{ $booking->flight->arrivalAirport->gate ?? 'A1' }}</p>
                    </div>
                    <div class="bg-green-50 rounded-xl p-4 text-center">
                        <p class="text-xs text-slate-500 font-medium">Boarding Time</p>
                        <p class="text-lg font-black text-green-700 mt-1">{{ \Carbon\Carbon::parse($booking->flight->departure_time)->subMinutes(30)->format('H:i') }}</p>
                ...
                @foreach($booking->passengers as $passenger)
                    <div class="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
                        ...
                        <div class="text-right">
                            ...
                            <div class="mt-1">
                                <span class="px-2 py-0.5 rounded text-xs font-medium {{ $passenger->check_in_status['class'] }}">{{ $passenger->check_in_status['label'] }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
