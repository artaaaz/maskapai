...
<div class="bg-gradient-to-br from-blue-700 via-blue-800 to-indigo-900 min-h-[280px] flex items-center relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10 py-8">
        <div class="flex items-center gap-2 mb-4">
            <a href="{{ route('customer.flights.detail', $flight) }}" class="text-white/60 hover:text-white transition-colors">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-white/70 text-sm mt-1">{{ $flight->departureAirport->city }} ({{ $flight->departureAirport->iata_code }}) → {{ $flight->arrivalAirport->city }} ({{ $flight->arrivalAirport->iata_code }})</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <span class="px-3 py-1.5 bg-yellow-400/20 backdrop-blur rounded-full text-yellow-300 text-xs font-bold">
                    {{ ucfirst(str_replace('_', ' ', $selectedClassName)) }}
                </span>
            </div>
        </div>
    </div>
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-[40px] relative z-20 pb-16">
    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
        ...
        <div class="p-4 md:p-6 border-b border-slate-100">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Pilih {{ $passengerCount }} kursi untuk penumpang</p>
                </div>
        ...
        {{-- Seat Map Form --}}
        <form action="{{ route('customer.flight-detail.store-seats', $flight) }}" method="POST" id="seatForm">
            @csrf
            <input type="hidden" name="flight_class_id" value="{{ $selectedFlightClass->id }}">
            <input type="hidden" name="passenger_count" value="{{ $passengerCount }}">
            <input type="hidden" name="travel_class" value="{{ $selectedClassName }}">
            
            {{-- Selected seats will be stored here --}}
            ...
            <div class="seat-map max-w-md mx-auto">
                @forelse($groupedSeats as $row => $seatsInRow)
                    <div class="seat-row">
                        <div class="seat-row-label">{{ $row }}</div>
                        @php $col = 0; @endphp
                        @foreach($seatsInRow as $seat)
                            {{-- Add gap after column 3 (aisle) --}}
                            @if($col === 3)
                                <div class="seat-gap"></div>
                            @endif
                            <div class="seat {{ $seat->status === 'booked' ? 'booked' : '' }} {{ in_array($seat->seat_number, old('seat_numbers', [])) ? 'selected' : '' }}"
                                 data-seat-number="{{ $seat->seat_number }}"
                                 data-status="{{ $seat->status }}"
                                 onclick="toggleSeat(this)"
                                 title="{{ $seat->seat_number }} - {{ $seat->status === 'booked' ? 'Terisi' : 'Tersedia' }}">
                                {{ $seat->seat_number }}
                            </div>
                            @php $col++; @endphp
                        @endforeach
                ...
                @empty
                    <div class="text-center py-8 text-slate-400">
                        Tidak ada kursi tersedia untuk kelas {{ ucfirst(str_replace('_', ' ', $selectedClassName)) }}.
                    </div>
                @endforelse
            </div>
