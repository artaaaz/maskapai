...
{{-- ===== FLOATING SEARCH PANEL BACKDROP ===== --}}
<div class="floating-panel-backdrop" id="panelBackdrop"></div>

{{-- ===== FLOATING SEARCH PANEL (FROM/TO) ===== --}}
    ...
    <div class="flex-1 overflow-y-auto panel-scroll px-6 pb-6">
        ...
        <div id="panelPopularSection">
            ...
            <div class="flex flex-wrap gap-2 mb-5" id="popularChips">
                @if(isset($airports))
                    @foreach($airports->take(10) as $ap)
                    <span class="dest-chip" data-city="{{ $ap->city }}" onclick="selectPopularDest('{{ $ap->city }}')">{{ $ap->city }} ({{ $ap->iata_code }})</span>
                    @endforeach
                @else
    ...
    <div class="px-6 py-5 space-y-5">
        ...
        <div>
            ...
            <div class="flex gap-2" id="cabinSelector">
                ...
                <button type="button" class="cabin-btn {{ $key == 'economy' ? 'active' : '' }}" data-class="{{ $key }}" onclick="selectCabin('{{ $key }}')">{{ $class['label'] }}</button>
                @endforeach
            </div>
        </div>
    </div>
...
<section class="relative min-h-[520px] md:min-h-[600px] lg:min-h-[660px] flex items-center justify-center overflow-hidden">
    <div class="absolute inset-0">
        <img src="{{ asset('images/2.jpg') }}" alt="Latar belakang destinasi penerbangan" class="w-full h-full object-cover" loading="eager">
    ...
    <div class="relative w-full max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 z-10 py-16 md:py-20 text-center">
        ...
        <div class="mb-6 md:mb-8">
            <h1 class="hero-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight">
                @auth Selamat datang, {{ Auth::user()->name }} @else Terbang ke <br>Destinasi Impian @endauth
            </h1>
            <p class="text-white/80 text-base md:text-lg mt-3 md:mt-4 mx-auto max-w-2xl">
                @auth Pesan tiket pesawat dengan mudah dan cepat. Harga terbaik, proses simpel. @else Temukan tiket pesawat terbaik untuk perjalanan Anda. Harga bersahabat, proses mudah. @endauth
            </p>
        </div>

        {{-- ===== SEARCH CARD MODERN (Tiket.com / Traveloka Style) ===== --}}
        <div class="search-card-modern p-4 md:p-6 w-full mx-auto max-w-[1100px]" role="search" aria-label="Cari penerbangan">
            <form action="{{ route('customer.flights.results') }}" method="GET" id="heroSearchForm" novalidate>
                {{-- Trip Type --}}
                ...
                <div class="flex flex-col lg:flex-row items-stretch lg:items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0" id="fromField">
                            ...
                            <select name="departure_airport_id" id="departureSelect" class="sr-only" aria-hidden="true">
                                ...
                                @foreach($airports as $ap)
                                    <option value="{{ $ap->id }}" data-city="{{ $ap->city }}" data-code="{{ $ap->iata_code }}">{{ $ap->city }} ({{ $ap->iata_code }})</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    {{-- Right side: Dates, Passengers, Search --}}
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-shrink-0">
                        ...
                        <div class="min-w-[130px]">
                            ...
                            <div class="search-field">
                                <input type="date" id="departDate" name="departure_date" required value="{{ now()->format('Y-m-d') }}"
                        ...
                        <div class="min-w-[130px] relative">
                            ...
                            <div class="search-field">
                                <input type="date" id="returnDate" value="{{ now()->addDay()->format('Y-m-d') }}"
                                       class="w-full h-12 px-4 bg-transparent border-none rounded-2xl text-sm text-black font-medium focus:outline-none"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            @foreach($popularRoutes as $route)
                ...
                @endphp
                <a href="{{ route('customer.flights.results') }}?departure_airport_id={{ $departure->id }}&arrival_airport_id={{ $arrival->id }}&departure_date={{ now()->format('Y-m-d') }}&passenger_count=1&travel_class=economy&trip_type=one_way"
                   class="dest-card block group" aria-label="Penerbangan {{ $departure->city }} ke {{ $arrival->city }}">
                    <img src="{{ $imgUrl }}" alt="{{ $arrival->city }}" class="dest-thumb" loading="lazy">
                    <div class="dest-overlay">
                        <p class="text-xs font-medium text-white/70">{{ $departure->city }} ({{ $departure->iata_code }}) →</p>
                        <p class="text-lg font-bold text-white">{{ $arrival->city }} ({{ $arrival->iata_code }})</p>
                        <p class="text-sm font-semibold text-white mt-1">Mulai Rp{{ number_format($route->min_price, 0, ',', '.') }}</p>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="card-modern bg-white border border-gray-100 p-5 flex items-center gap-4 shadow-sm">
                ...
                <div><p class="text-2xl font-bold text-black">{{ $myBookings ?? 0 }}</p><p class="text-xs text-gray-500">Total Booking</p></div>
            </div>
            <div class="card-modern bg-white border border-gray-100 p-5 flex items-center gap-4 shadow-sm">
                <div class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0">
                ...
                <div><p class="text-2xl font-bold text-black">{{ $myTrips ?? 0 }}</p><p class="text-xs text-gray-500">Selesai</p></div>
            </div>
            <div class="card-modern bg-white border border-gray-100 p-5 flex items-center gap-4 shadow-sm">
                <div class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0">
                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
                <div><p class="text-2xl font-bold text-black">Rp{{ number_format($mySpent ?? 0, 0, ',', '.') }}</p><p class="text-xs text-gray-500">Pengeluaran</p></div>
            </div>
        </div>
    </div>
</section>
...
<section class="py-14 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-8 flex-wrap gap-4">
            ...
            @if(isset($allFlights) && $allFlights->count() > 0)
                <span class="text-sm font-semibold text-blue-600 bg-blue-50 px-4 py-2 rounded-full">{{ $allFlights->count() }} penerbangan</span>
            @endif
        </div>
        @if(isset($allFlights) && $allFlights->count() > 0)
                    ...
                    <div class="flex items-center justify-between text-white">
                        <div class="flex items-center gap-2">
                            @if($flight->airline->logo)
                                <img src="{{ asset('storage/' . $flight->airline->logo) }}" alt="{{ $flight->airline->name }}" class="w-7 h-7 rounded-lg object-cover border border-white/20">
                            @else
                        ...
                        @else
                            <span class="text-xs font-medium bg-white/20 px-3 py-1 rounded-full">{{ $availSeats }} kursi</span>
                        @endif
                    </div>
                </div>
                <div class="p-5">
                    <div class="flex items-center justify-between mb-4">
                        <div class="text-center"><p class="text-xl font-bold text-black">{{ \Carbon\Carbon::parse($flight->departure_time)->format('H:i') }}</p><p class="text-xs text-gray-400">{{ $flight->departureAirport->iata_code ?? '' }}</p></div>
                        <div class="flex-1 mx-3"><div class="h-px bg-gray-200 relative"><div class="w-3 h-3 bg-blue-600 rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"></div></div><p class="text-xs text-gray-400 text-center mt-1">{{ $flight->duration_formatted }}</p></div>
                        <div class="text-center"><p class="text-xl font-bold text-black">{{ \Carbon\Carbon::parse($flight->arrival_time)->format('H:i') }}</p><p class="text-xs text-gray-400">{{ $flight->arrivalAirport->iata_code ?? '' }}</p></div>
                    </div>
                    <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                        <div><p class="text-xs text-gray-400">Harga/orang</p><p class="text-xl font-bold text-black">Rp{{ number_format($price, 0, ',', '.') }}</p></div>
                        @if($isFull)
                            <span class="px-5 py-2.5 bg-gray-300 text-gray-500 font-semibold rounded-xl text-sm cursor-not-allowed">Penuh</span>
                        @else
                            <a href="{{ route('customer.flights.detail', $flight) }}?passenger_count=1&travel_class=economy" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition text-sm" aria-label="Lihat detail penerbangan {{ $flight->flight_number }}">Detail</a>
                        @endif
                    </div>
                </div>
            </div>
...
<section class="py-14 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-8 md:p-12 text-center shadow-xl shadow-blue-600/20">
            ...
            <a href="{{ route('customer.flights.results') }}" class="inline-flex items-center gap-2 px-8 py-3 bg-white text-blue-600 font-semibold rounded-xl hover:bg-blue-50 transition shadow-lg" aria-label="Cari dan pesan penerbangan sekarang">Pesan Sekarang</a>
        </div>
    </div>
</section>
...
(function() {
    ...
    const airportSearchUrl = '{{ route('customer.airports.search') }}';
