...
<head>
    ...
    <title>{{ $title ?? 'Staff - drgMaskapai' }}</title>
                ...
                @foreach($menuItems as $item)
                    ...
                    <a href="{{ route($item['route']) }}"
                       class="flex items-center gap-3 px-4 py-3 rounded-xl transition
                              {{ $isActive ? 'bg-white/10 text-white font-medium' : 'text-emerald-100 hover:bg-white/5 hover:text-white' }}">
        ...
        <div class="flex-1 overflow-y-auto">
            <div class="p-8">
                ...
                <div class="flex justify-between items-center mb-8">
                    <div>
                        <h2 class="text-2xl font-bold text-slate-800">{{ $header ?? 'Dashboard' }}</h2>
